(function () {
    ...
    function init() {//tudo necessario para iniciar o jogo
        ...
        m.innerHTML= `Manual:<br>Aperte "↓" para acelerar<br>Aperte "↑" para desacelerar<br>Aperte "←" para ir para esquerda<br>Aperte "→" para ir para direita<br>Aperte "F" para alternar a velocidade`;
    ...
    function morreu(){
        ...
        fim_de_jogo();
        montanha.element.style.backgroundColor = '#40E0D0';
    }
    ...
    window.addEventListener('keydown', (e) => {//muda a velocidade com setas
        if(trava==0){
            if((e.key ==='ArrowDown')){
                ...
                clearInterval(gameLoop);
                gameLoop = setInterval(run, 1000/FPS);
                //console.log('Rapidao: '+FPS);
            }else if((e.key ==='ArrowUp')&&(FPS>15)){
                FPS-=25;
                clearInterval(gameLoop);
                gameLoop = setInterval(run, 1000/FPS);
                //console.log('Lentao: '+FPS);
            }
        }
    })
    ...
    class Placar{
        constructor () {
            this.element = document.createElement('div');//criar div
            montanha.element.appendChild(this.element);//colocar id arvore no div
            this.element.id = 'placar';//div na placar dom
        ...
        incremento_e_vida(){//metros percorridos
            ...
            document.getElementById("placar").innerHTML = `Placar: ${parseInt(++(aux_ms)/80)} m/s <br> Vida: ${value_vida}`;
            //console.log(aux_ms +"m/s")
        }
    }
    ...
    function run() {//funcao que vai fazer tudo acontecer no jogo
        ...
        cogumelos.forEach((c) => {//batida na cogumelo
            if(parseInt(c.element.style.top) ==30){
